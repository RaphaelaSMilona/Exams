...
#include <string.h>
#include <ctype.h>
// This program searches for and prints DNA or RNA coding regions of prokaryotic genes, in entries of max 3000 bp, and contains annotations
int main()
{
				...
				for(i=0; i<=lengthseq - 2; i++)
				{
					if (seq[i] == 'A' && (seq[i + 1] == 'T'|| seq[i + 1] == 'U')  && seq[i + 2] == 'G') //start codon
                                	       		...
                                	       		if(strcmp(codon,"TGA") == 0 || strcmp(codon, "TAA") == 0 || strcmp(codon,"TAG")== 0 || strcmp(codon,"UAG")== 0 || strcmp(codon,"UAA")== 0 || strcmp(codon,"UGA")== 0) //strcmp compares the  lexical relationship of two string and returns 0 when they are the same
							{
									...
									{
										if(strlen(gene)>100) // typically genes are larger than 100nt
										{
